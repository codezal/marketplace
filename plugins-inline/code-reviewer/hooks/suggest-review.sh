#!/usr/bin/env bash
# code-reviewer — opt-in proactive nudge.
#
# Fires after Write/Edit/MultiEdit. SILENT by default: does nothing unless
# CODE_REVIEWER_AUTO is set to a non-empty value. When enabled, it only
# *suggests* running /review on the edited source file — it never reads,
# reviews, or modifies your code. Safe autonomous channel, zero surprises.
set -eu

# Opt-in gate. Default behavior is to do nothing at all.
if [ -z "${CODE_REVIEWER_AUTO:-}" ]; then
  exit 0
fi
...
msg="code-reviewer: \`${base}\` changed. Run /review to review the diff, or /review-deep for a multi-perspective audit."
